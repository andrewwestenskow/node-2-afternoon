...
export default class ChatWindow extends Component {
  ...
  createMessage(event) {
    ...
    if (event.key === "Enter" && text.length !== 0) {
      axios.post(url, { text, time: dateCreator(), name: this.state.username }).then(response => {
        this.setState({ messages: response.data });
      });
          ...
          <div id="ChatWindow__messagesChildContainer">
            {
              this.state.messages.map(message => (
                ...
                name={message.name}
                  text={message.text}
                  time={message.time}
                  edit={this.editMessage} remove={this.removeMessage} />
              ))
            }
          </div>
        </div>
        <div id="ChatWindow__newMessageContainer">
        <input placeholder="Your name" className="NameHolder" onChange={this.handleNameChange}/>
